...
from services.auth_service.app.mail import MailServiceError, send_signup_otp_email
from services.auth_service.app.repository import (
    AuthRepositoryError,
    create_refresh_token,
    create_signup_session,
...
def _new_refresh_token_for_user(user_id: str) -> tuple[str, str]:
    refresh_token_plain = create_refresh_token_plain()
    refresh_token_hash = hash_refresh_token(refresh_token_plain)
    create_refresh_token(
        user_id=user_id,
    ...
    return refresh_token_plain, refresh_token_hash


def _issue_tokens(user: dict[str, Any]) -> dict[str, Any]:
    access_token = create_access_token(user)
    refresh_token_plain, _refresh_token_hash = _new_refresh_token_for_user(str(user["user_id"]))
    return {
        "access_token": access_token,
        "refresh_token": refresh_token_plain,
        "token_type": "bearer",
...
def _require_public_signup_allowed(payload: SignupStartRequest) -> None:
    if payload.role != "farmer":
        raise AuthServiceError("Public signup is only allowed for farmer users")
    if not payload.consent_terms:
        raise AuthServiceError("Terms and privacy consent is required")


def start_signup(payload: SignupStartRequest) -> dict[str, Any]:
    _require_public_signup_allowed(payload)
    email = str(payload.email).strip().lower()

    if get_user_by_email(email) is not None:
        raise AuthServiceError("A user already exists with this email")

    otp = f"{secrets.randbelow(900000) + 100000}"
    expires_at = datetime.now(timezone.utc) + timedelta(minutes=settings.signup_otp_expire_minutes)
    ...
    try:
        ...
        send_signup_otp_email(email, payload.full_name, otp)
    except (AuthRepositoryError, MailServiceError) as exc:
        raise AuthServiceError(str(exc)) from exc

    response = {
        "signup_session_id": session["signup_session_id"],
        "otp_expires_in_seconds": settings.signup_otp_expire_minutes * 60,
        "dev_otp": None,
    }
    if settings.app_env == "local" and settings.mail_provider.strip().lower() == "console":
        response["dev_otp"] = otp
    return response
...
def verify_signup_otp(payload: SignupVerifyRequest) -> dict[str, Any]:
    ...
    if session is None:
        raise AuthServiceError("Signup session not found")
    if session.get("completed_at"):
        raise AuthServiceError("Signup session is already completed")
    if session.get("locked_at"):
        raise AuthServiceError("Too many invalid OTP attempts. Start signup again.")
    if session.get("verified_at"):
        return {"signup_session_id": payload.signup_session_id, "verified": True}
    if session["otp_expires_at"] <= datetime.now(timezone.utc):
        raise AuthServiceError("OTP has expired. Start signup again.")

    attempts = int(session.get("attempts") or 0)
    if not verify_password(payload.otp, session["otp_hash"]):
        should_lock = attempts + 1 >= settings.signup_otp_max_attempts
        increment_signup_attempts(payload.signup_session_id, lock=should_lock)
        raise AuthServiceError("Invalid OTP")
...
def login_with_google(id_token_value: str) -> dict[str, Any]:
    try:
        ...
        identity = get_auth_identity(provider, provider_subject)
...
def refresh(refresh_token_plain: str) -> dict[str, Any]:
    old_token_hash = hash_refresh_token(refresh_token_plain)
    refresh_row = get_active_refresh_token(old_token_hash)
    if refresh_row is None:
        raise AuthServiceError("Invalid or expired refresh token")
    ...
    access_token = create_access_token(user)
    new_refresh_token_plain, new_refresh_token_hash = _new_refresh_token_for_user(str(user["user_id"]))
    revoke_refresh_token(old_token_hash, replaced_by_token_hash=new_refresh_token_hash)
...
def logout(refresh_token_plain: str) -> dict[str, bool]:
    revoke_refresh_token(hash_refresh_token(refresh_token_plain))
    return {"logged_out": True}
