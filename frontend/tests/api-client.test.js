...
describe("api client authentication behaviour", () => {
  ...
  it("does not refresh or redirect for a public wrong-password 401", async () => {
    let refreshCalls = 0;
    server.use(
      http.post(`${BASE}/v1/auth/login`, () => HttpResponse.json({ detail: { code: "INVALID_LOGIN", message: "Invalid login credentials." } }, { status: 401 })),
      http.post(`${BASE}/v1/auth/refresh`, () => { refreshCalls += 1; return HttpResponse.json({}, { status: 401 }); }),
    );
  ...
  it("shares one refresh across concurrent protected 401 responses", async () => {
    ...
    server.use(
      ...
      http.get(`${BASE}/v1/a`, ({ request }) => request.headers.get("authorization") === "Bearer new-access" ? HttpResponse.json({ ok: "a" }) : HttpResponse.json({}, { status: 401 })),
      http.get(`${BASE}/v1/b`, ({ request }) => request.headers.get("authorization") === "Bearer new-access" ? HttpResponse.json({ ok: "b" }) : HttpResponse.json({}, { status: 401 })),
    );
