...
export default function PhotoPicker({ value = [], onChange, locale, title, maxImages = MAX_IMAGES }) {
  ...
  async function handleFiles(event) {
    ...
    try {
      ...
      onChange([
        ...value,
        ...compressed.map((blob, index) => new File([blob], selected[index].name, { type: blob.type })),
      ]);
    } catch {
    ...
    <div>
      ...
      <div className="mt-2 flex gap-2.5 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {Array.from({ length: maxImages }).map((_, index) => {
          ...
          if (file) {
            return (
              <div key={`${file.name}-${index}`} className="relative h-[84px] w-[84px] shrink-0 overflow-hidden rounded-[14px] border border-[#E9E7DC]">
                <Preview file={file} />
                <button type="button" onClick={() => onChange(value.filter((_, i) => i !== index))} className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-black/60 text-white" aria-label={primary(STRINGS.delete, locale)}>
                  <X className="h-3 w-3" />
                </button>
              </div>
            );
          ...
          return (
            <button key={`empty-${index}`} type="button" onClick={() => inputRef.current?.click()} disabled={busy} className="flex h-[84px] w-[84px] shrink-0 flex-col items-center justify-center gap-1 rounded-[14px] border-2 border-dashed border-[#E9E7DC] bg-[#F7F8F3] text-[11px] font-bold text-[#5B6055] active:scale-[0.98] disabled:opacity-60" aria-label={primary(STRINGS.addPhoto, locale)}>
