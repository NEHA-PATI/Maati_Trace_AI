...
export const ANALYSIS_PIPELINE_STEPS = [
  ["farm_ready", "Prepare farm metadata"],
  ...ENVIRONMENT_DATASETS.map(([key, label]) => [`environment:${key}`, label]),
  ["sentinel2_history_backfill", "Prepare satellite history"],
  ["trends", "Compute farm trends"],
  ["grid_context", "Prepare display grid"],
  ["intelligence", "Build crop intelligence"],
];
